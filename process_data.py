...
def process_file(path,company,report,history):
	...
	for j in range(len(sheets)):
		...
		chars=pd.concat([pd.Series(sheets[j].columns).rename('Field'),pd.Series(count_chars).rename('Count')],axis=1)
		...
		#print('Number of records:'.upper(),'-'*len('Number of records:'), str(np.shape(sheets[j])[0]),'','Field Names and Maximum number of Characteres per field:'.upper(),'-'*len('Field Names:'),pd.Series(sheets[j].columns),'',
		#	  sep='\n',file=open(report +'/issues.txt','a'))
		...
		itsm_columns=['Site*','Region','Site Group','City*']
		...
		if len(glob.glob(history+'/*'))==0:
			...
			sites_itsm=pd.read_excel(glob.glob(history+'/*')[0],pd.ExcelFile(glob.glob(history+'/*')[0]).sheet_names[0])[:-2]
			sites_itsm[itsm_columns]
		...
		if sheets[j].columns.str.contains('CI N',case=False).any():
			...
			if (np.shape(filtered_dns)[1]==1):
				...
				if well_conc_dns==False:
					cis[0][filtered_dns.columns[0]]=cis[0][filtered_cis.columns[0]]
				else:
					None
			else:
				well_conc_dns='Missing or wrong name identifiers of the following fields: CI Name, DNS Host Name'
			print('DNS Host Name well concatenated:'.upper(),'-'*len('DNS Host Name well concatenated:'),'With CI Name:',well_conc_dns,'',sep='\n',file=open(report +'/issues.txt','a'))       
			##duplicates
			duplicate_cis=[]
	...
	with pd.ExcelWriter(report + '/' + company + '_report_'+ datetime.now().strftime("%Y-%m-%d %H-%M-%S") +'.xlsx',engine='xlsxwriter') as writer:
